...
import 'todo_bottom_textfield.dart';
...
class _TodoContentState extends State<TodoContent> {
  ...
  void _onBottomFieldAdded(String value) {
    ...
    context.read<NoteBloc>().add(NoteTodoItemAdded(
          item: TodoItem(text: value),
        ));
  }
  ...
  void _onItemDeleted(List<TodoItem> items, TodoItem item) {
    ...
    context.read<NoteBloc>().add(NoteTodoItemDeleted(id: item.id));
  }
              ...
              children: [
                for (final item in items)
                  TodoTile(
                    ...
                    onTextChanged: (text) => context.read<NoteBloc>().add(
                        NoteTodoItemChanged(item: item.copyWith(text: text))),
